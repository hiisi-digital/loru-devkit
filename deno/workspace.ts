...
async function collectNested(configs: WorkspaceConfig[], seen: Set<string>, baseDir: string, config: LoruConfig) {
  for (const member of config.workspace?.members ?? []) {
    ...
    configs.push({ path: loaded.path, baseDir: loaded.baseDir, config: loaded.config });
